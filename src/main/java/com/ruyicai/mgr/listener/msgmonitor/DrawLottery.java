...
@Component
public class DrawLottery extends AbsMsgMonitor{
	public static Map<String, Long> map = null;
	@Override
	public void process() {
		if (map == null) {
			map = new HashMap<String, Long>();
			List<TmsgMonitor> list = TmsgMonitor.findAllTmsgMonitorsByType(0);
			for (TmsgMonitor msgMonitor : list) {
				map.put(msgMonitor.getId().getLotno(), msgMonitor.getTime().longValue());
			}
		}
		String lotno;
		Iterator<Entry<String, Long>> i = TlotListener.kaijiang.entrySet().iterator();
		while (i.hasNext()) {
			...
			if (System.currentTimeMillis() - e.getValue() > map.get(lotno)) {
				String msgBody  = "采种编号：" + lotno +"，期号:"+e.getKey().split("_")[1]+"开奖后有" + 
				(System.currentTimeMillis()- e.getValue())/60000L + "分钟没派奖了";
				sendmsg(msgBody);
				i.remove();
			}
		}
