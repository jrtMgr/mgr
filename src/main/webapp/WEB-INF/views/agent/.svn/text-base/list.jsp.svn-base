...
$(document).ready(function() {
	...
	$.each(data,function(i,n){
		tbody.append('<tr>');
		tbody.append('<td></td>');
		tbody.append('<td title='+data[i].tuserinfo.userno+'>'+data[i].tuserinfo.userno+'</td>');
		tbody.append('<td title='+data[i].tuserinfo.userName+'>'+data[i].tuserinfo.userName+'</td>');
		tbody.append('<td>'+data[i].userAgency.parentUserno+'</td>');
		tbody.append('<td title='+data[i].userAgency.level+'>'+data[i].userAgency.level+'</td>');
		tbody.append('<td title='+data[i].userAgency.state+'>'+data[i].userAgency.state+'</td>');
		var da = new Date(data[i].userAgency.createTime);
		var daStr = da.format("yyyy-MM-dd hh:mm:ss");
		tbody.append('<td title='+daStr+'>'+daStr+'</td>');
		tbody.append('<td><a href="javascript:process(\''+data[i].userAgency.userno+'\')">查看</a></td>');
		tbody.append('</tr>')
	});
});

function process(id){
	var url = "<%=request.getContextPath()%>/agent/commisionRatio?userno="+id;
	location.href=url;
}
...
<body>
	...
	<table width="100%" cellspacing="0" cellpadding="0" border="0">
		<tbody>
						...
						<div style="float: left;">
							<table width="130%" cellspacing="2" cellpadding="2" border="0">
								<tr>
									...
									<form name="submitForm" action="<%=request.getContextPath()%>/agent/page" method="post">
									<td><input name="userno" type="text" style="width: 120px" value="${param.userno}"></td>
									
									<td align="right">level:</td>
									<td><input name="level" type="text" style="width: 120px" value="${param.level}"></td>
					...
					<table width="100%" cellspacing="0" cellpadding="2"
						class="dataTable">
							<tr class="dataTableHead">
								...
								<td width="2%" class="thOver"><strong>用户编号</strong></td>
								<td width="2%" class="thOver"><strong>用户名</strong></td>
								<td width="4%" class="thOver"><strong>上级用户编号</strong></td>
								<td width="1%" class="thOver"><strong>level</strong></td>
								<td width="1%" class="thOver"><strong>状态</strong></td>
								<td width="4%" class="thOver"><strong>创建时间</strong></td>
								<td width="4%" class="thOver">佣金比例</td>
							</tr>
							<tbody id="data">
							</tbody>
							<tr>
			...
			<tr><td style="padding-top: 2px; padding-left: 6px; padding-right: 6px; padding-bottom: 2px; font-size: 14px;"><a href="<%=request.getContextPath()%>/agent/addUI">新增用户</a></td></tr>
		</tbody>
	</table>
</body>
</html>