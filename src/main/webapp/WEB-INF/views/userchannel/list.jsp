...
<script type="text/javascript" >
	function updateChannel(userno){
		var channel = document.getElementById(userno).value;
		var url = "<%=request.getContextPath()%>/userchannel/updateUserChannel?userno="+userno+"&channel="+channel;
		...
		<tbody>
			<tr>
				<td style="padding: 2px 10px;">
					<form action="<%=request.getContextPath()%>/userchannel/list" method="post">
					...
					<table width="100%" cellspacing="0" cellpadding="2"
						...
						<tbody>
							<tr class="dataTableHead">
								...
								<td width="8%" class="thOver"><strong>用户名</strong>
								</td>
								<td width="4%" class="thOver"><strong>邮箱</strong>
								</td>
								<td width="8%" class="thOver"><strong>身份证</strong>
								</td>
								<td width="12%" class="thOver"><strong>渠道号</strong>
								</td>
							...
							<c:if test="${!empty page.list}">
								<c:forEach items="${page.list}" var="user" varStatus="num">
									<tr>
										...
										<td title="${user.userno}">${user.userno}</td>
										...
										<td ><input style="width: 30px;" type="text" value="${user.channel}" id="${user.userno}"/>
										<a style="cursor:hand;" onclick="javascript:updateChannel('${user.userno}')" >修改</a></td>
										<td title="${user.subChannel}">${user.subChannel}</td>
										<td>
											<% Tuserinfo userinfo = (Tuserinfo) pageContext.getAttribute("user"); %>
