...
<%@page import="com.ruyicai.mgr.domain.statis.Channel"%>
...
<body>
	...
	<form action="<%=request.getContextPath()%>/tactivities/add" method="post">
					...
					<table width="100%" cellspacing="0" cellpadding="2"
						...
						<tbody>
							...
							<tr>
							    ...
							    <td width="42%" align="left"><label for="lblid">${id}</label><input name="id" type="hidden" class="inputText" style="width: 120px" id="id"  value="${id}" /></td>
							...
							<tr>
							    ...
							    <td width="8%" align="right">渠道:</td>
							    <td width="42%" align="left">
							           <!--<input name="channel" type="text" style="width: 120px" 
										id="channel" class="inputText" onfocus="this.select();" value="${channel}"/>-->
										<select name="channel" style="width:120px" id="channel">
											<option value="">无</option>
											<c:forEach
												items="<%=com.ruyicai.mgr.domain.statis.Channel.getMap().entrySet() %>"
												var="channelvar">
												<option value="${channelvar.key}" <c:if test="${channelvar.key eq channel}">selected</c:if>>${channelvar.value}</option>
											</c:forEach>
									</select>
