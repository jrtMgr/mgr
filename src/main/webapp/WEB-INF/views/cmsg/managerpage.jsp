...
$(document).ready(function() {
	...
	$.each(data,function(i,n){
		var id = data[i].id;
		var url = "<%=request.getContextPath()%>/cmsg/mpage?pid="+id;
		tbody.append('<tr>')
		tbody.append('<td></td><td>'+id+'</td>');
		tbody.append('<td title='+data[i].userno+'>'+data[i].userno+'</td>');
		if(data[i].reply == null){
			tbody.append('<td title='+data[i].reply+'><a href="javascript:huifu('+id+')">回复</a></td>');
		}else{
			tbody.append('<td title='+data[i].reply+'><a href="javascript:huifu('+id+')" >已回复</a></td>');
		}
...
function huifu(id){
	...
	window.open("<%=request.getContextPath()%>/cmsg/huifuUI?id="+id,"yhgl_edit",fea_edit);
}
						...
						<div style="float: left;">
							<table width="130%" cellspacing="2" cellpadding="2" border="0">
								<tr>
									...
									<form name="submitForm" action="<%=request.getContextPath()%>/cmsg/mpage" method="post">
					...
					<table width="100%" cellspacing="0" cellpadding="2"
						class="dataTable">
							<tr class="dataTableHead">
								...
								<td width="4%" class="thOver"><strong>用户编号</strong></td>
								<td width="3%" class="thOver"><strong>回复</strong></td>
								<td width="22%" class="thOver"><strong>留言内容</strong></td>
								<td width="17%" class="thOver"><strong>客户详细</strong></td>
								<td width="7%" class="thOver"><strong>留言时间</strong></td>
								<td width="7%" class="thOver"><strong>imsi</strong></td>
							</tr>
							<tbody id="data">
							</tbody>
							<tr>
