		...
		<tbody>
			<tr>
				<td style="padding: 2px 10px;">
					<form action="<%=request.getContextPath()%>/clientdaily/list" method="post">
					...
					<table width="100%" cellspacing="0" cellpadding="2"
						...
						<tbody>
							<tr class="dataTableHead">
								...
								<td width="30%" class="thOver"><strong>渠道名</strong></td>
								<td width="30%" class="thOver"><strong>渠道ID</strong></td>
								<td width="30%" class="thOver"><strong>数量</strong></td>
							</tr>
							<% 
								...
								<td align="left" id="dg1_PageBar" colspan="17"><div
										...
										<c:choose>
											<c:when test="${page.pageIndex != 0}">
												<a
													href="<%=request.getContextPath()%>/clientdaily/list?channelid=${param.channelid}&maxResult=${page.maxResult}&pageIndex=0">第一页</a>
											</c:when>
											<c:otherwise>
										第一页
									</c:otherwise>
										...
										<c:choose>
											<c:when test="${page.pageIndex > 0}">
												<a
													href="<%=request.getContextPath()%>/clientdaily/list?channelid=${param.channelid}&maxResult=${page.maxResult}&pageIndex=${page.pageIndex - 1}">上一页</a>
											</c:when>
											<c:otherwise>
										上一页
									</c:otherwise>
										...
										<c:choose>
											<c:when test="${page.pageIndex + 1 < page.totalPage}">
												<a
													href="<%=request.getContextPath()%>/clientdaily/list?channelid=${param.channelid}&maxResult=${page.maxResult}&pageIndex=${page.pageIndex + 1}">下一页</a>
											</c:when>
											<c:otherwise>
										下一页
									</c:otherwise>
										...
										<c:choose>
											<c:when test="${page.pageIndex + 1 != page.totalPage}">
												<a
													href="<%=request.getContextPath()%>/clientdaily/list?channelid=${param.channelid}&maxResult=${page.maxResult}&pageIndex=${page.totalPage - 1}">最末页</a>
											</c:when>
											<c:otherwise>
										最末页
									</c:otherwise>
										...
										<script type="text/javascript">
											function go() {
												...
												window.location.href="<%=request.getContextPath()%>/clientdaily/list?channelid=${param.channelid}&maxResult=${page.maxResult}&pageIndex=" + pageindex;
											}
										</script>
									</div>
