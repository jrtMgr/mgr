...
function openinfo(name) {
	...
	var d = new Dialog("开奖信息", "gentlemanopen/openinfo?info=" + ($("#" + name + "_batchcode").val()), 400, 300);
	d.show();
}
function info(name) {
	var info = $("#" + $("#" + name + "_batchcode").val()).val().split("_");
	...
	var d = new Dialog("开奖信息", "gentlemanopen/info?info=" + ($("#" + name + "_batchcode").val()), 800, 450);
	d.show();
}
</script>
<body>
					...
					<table width="100%" cellspacing="0" cellpadding="2"
						...
						<tbody>
							<tr class="dataTableHead">
								...
								<td width="9%" class="thOver"><strong>开奖</strong>
								</td>
							...
							<c:forEach items="<%=tlotctrls.entrySet() %>" var="entry"
								...
								<tr>
									...
									<td id="${entry.key}_open" align="center">
										<a href="javascript:openinfo('${entry.key}');">运维开奖</a>
